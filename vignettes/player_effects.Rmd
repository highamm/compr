...
The `compr_james` package in `R` provides a framework for fitting paired competition models. The primary function within this package, `comp_glm()`, enables users to model competition outcomes. One of the key features of this function is the ability to incorporate individual player effects through the `p1_effects` and `p2_effects` arguments.
