...
```{r}
combined_df <- combined_df[1:1230, ]
combined_df <- combined_df[, -c(2, 7, 9, 10, 11)]
...
combined_df <- combined_df %>% #there are both OT and 2OT
  mutate(OT = ifelse(OT == "", FALSE, TRUE),
         home_winner = ifelse(home_pts > away_pts, TRUE, FALSE))

save(combined_df, file = "data/nba_game.rda")
```
